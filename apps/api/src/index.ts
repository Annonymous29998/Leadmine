...
app.get('/api/health', async () => ({ ok: true, name: 'LeadMine Extractor' }));
...
/** Live log stream for the Progress page */
...
/* ——— Sniffy-compatible job API (single-page live extractor) ——— */
...
// Also expose Sniffy-style paths without /api prefix (when served as single app)
for (const [method, pathName, handler] of [
  ['POST', '/start_extraction', 'start'],
  ['GET', '/get_progress', 'progress'],
  ['POST', '/stop_extraction', 'stop'],
  ['GET', '/download_results', 'download'],
] as const) {
  // registered above under /api — aliases added below after static
  void method;
  void pathName;
  void handler;
}
